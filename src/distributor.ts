...
import {FluenceClient, generatePeerId, peerIdToSeed, seedToPeerId, uploadModule} from "@fluencelabs/fluence";
...
export class Distributor {
	...
	async uploadBlueprint(node: Node, bp: Blueprint): Promise<Blueprint> {
		...
		const blueprintId = await client.addBlueprint(bp.name, bp.dependencies, bp.id, node.peerId, TTL);
	...
	async createService(node: Node, bp: Blueprint): Promise<string> {
		// TODO
		/*const client = await this.makeClient(node);
		log.warn(`creating service ${bp.name}@${bp.id}`);
		let serviceId = await client.createService(bp.id, node.peerId, TTL);
		log.warn(
